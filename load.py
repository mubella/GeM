...
def traj_loc(traj,poi):

    traj=np.array(traj)

    print(traj.shape)
    shap = traj.shape
    traj_ = np.zeros((shap[0], shap[1], 5))# u,l,t,x,y,

    poi_index=poi[:,0]
    for idex1,user in enumerate(traj):
        for idex2,item in enumerate(user) :
          a=poi[np.where(poi_index==item[1])]
          if (item[1]==0):
              a=np.zeros((1,3))
          item=np.append(item,a[0,1:3])
          traj_[idex1,idex2]=item
    return traj_
...
def process_traj(dname):  # start from 1
    ...
    trajs_pre = traj_loc(trajs, poi)  #ultxy
    print('trajs_pr',trajs_pre.shape)
    data_pkl = './data_graph/' + dname + '_ultxy_data.pkl'
    # open(data_pkl, 'a')
    with open(data_pkl, 'wb') as pkl:
        joblib.dump(trajs_pre, pkl)
...
def load_graph_node_features(path, feature1='checkin_cnt', feature2='poi_index',feature3='poi_deltat',
                             feature4='latitude', feature5='longitude'):
    """X.shape: (num_node, 4), four features: checkin cnt, poi cat, latitude, longitude"""
    df = pd.read_csv(path)
    #print('df[node_name/poi_id]',df['node_name/poi_id'])
